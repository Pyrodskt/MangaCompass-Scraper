...
class Mangas:
    ...
    def write_file(self):
        ...
        print("writing file with new data")
        with open("data/data.json", "w") as fichier:
            fichier.write(json.dumps(content))
        
    def read_file(self):
        with open("data/data.json", "r") as fichier:
    ...
    def updateMangas(self):
        for i in self.mangas:
            print("starting update for ", i.url)
            i.start()
            
        for i in self.mangas:
            i.join()

        print("update finished for all threads")   
        self.write_file()
        self.updated = True
